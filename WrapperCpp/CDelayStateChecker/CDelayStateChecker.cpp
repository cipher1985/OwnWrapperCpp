...
CDelayStateChecker::CDelayStateChecker() :
    m_lastUpdateTime(std::chrono::steady_clock::now()) {}

void CDelayStateChecker::SetCheckFunc(
...
void CDelayStateChecker::UpdateData(int value)
{
    if(!m_funcStateCheck)
        return;
...
void CDelayStateChecker::SetDelayTime(int64_t ms)
{
    m_delayMS = ms;
    m_lastUpdateTime = std::chrono::steady_clock::now();
}

void CDelayStateChecker::SetState(int state)
{
    m_state = state;
    m_lastUpdateTime = std::chrono::steady_clock::now();
}
